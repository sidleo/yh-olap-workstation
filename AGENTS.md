...
- 2026-09 **编辑器滚动错位根治（hl/gutter 高=scrollHeight + curline 用 mirror 实测）**：用户复现"滚动后代码比光标滚得快，50 行内容滚动后代码/高亮/背景落在 49 行，但光标在 50 行正确"。根因两层：①hl-inner 与 gutter 的内容高用 `visualRowsOf` 手算折行（等宽估算），与浏览器真实 `pre-wrap` 折行有 subpixel 偏差、随行数累积 → 内容高不足 → transform 滚动位移后错位（"滚更快"= hl 内容先耗尽）；②curline(当前行背景) top 用 `cursorXY` 手算 → 光标前内容被少算一行 → 背景落上一行。修复：①syncHl 每次把 gutter 与 hl-inner 高撑到 `ta.scrollHeight`（浏览器真实内容高），gutter CSS 去 `bottom:0` 让高度可被 JS 撑起 → 滚动位移像素精确；②curTop 改 mirror 实测（建同宽同 padding 的隐藏 div、浏览器真实折行、测 marker.offsetTop 得光标行内容坐标 y，公式 `my+mh/2-paddingTop`）。**经验：手算"等宽累积折行"永远无法精确模拟浏览器 pre-wrap（字体 subpixel/断行规则）——凡需与 textarea 逐像素对齐的层（高亮/行号/当前行背景），内容高一律用 ta.scrollHeight 校准、光标行位置用 mirror 实测，不要用手算折行驱动滚动同步；curline 坐标系注意：它位于被 transform 位移的 hl-inner 内，top 用内容坐标而非 caretPos 的视口坐标（后者已减 scrollTop）。**
